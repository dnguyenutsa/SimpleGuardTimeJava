/*
 * $Id: BaseX.java 169 2011-03-03 18:45:00Z ahto.truu $
 *
 *
 *
...
public class BaseX {
	 ...
	 * form. The indices {@code 0..value.length-1} correspond to code points
	 * {@code min..max}.
	 */
	private int[] values;
	...
	public BaseX(String alphabet, boolean caseSensitive, char padding) {
		...
		if ((1 << bits) != alphabet.length()) {
			throw new IllegalArgumentException(
					"The size of the encoding alphabet is not a power of 2");
		}
		...
		if (padding >= min && padding <= max && values[padding - min] != -1) {
			throw new IllegalArgumentException(
					"The padding character appears in the encoding alphabet");
		}
		pad = padding;
	}
	...
	private void addMinMax(String chars) {
		for (int i = 0; i < chars.length(); i++) {
			int c = chars.charAt(i); // use `codePointAt` if Java 1.4.2 support is not needed.
			if (min == -1 || min > c) {
				min = c;
			}
			if (max == -1 || max < c) {
	...
	private void addChars(String chars) {
		for (int i = 0; i < chars.length(); i++) {
			int c = chars.charAt(i) - min; // use `codePointAt` if Java 1.4.2 support is not needed.
			if (values[c] != -1 && values[c] != i) {
				throw new IllegalArgumentException(
						"Duplicate characters in the encoding alphapbet");
			}
			values[c] = i;
		}
	}
	 ...
	 */
	public StringBuffer encode(byte[] in, String sep, int freq) {
	 ...
	 * @return a newly allocated buffer containing the encoded data.
	 */
	public StringBuffer encode(byte[] in, int off, int len, String sep, int freq) {
		...
		if (sep == null) {
			...
			for (int i = 0; i < sep.length(); i++) {
				int c = sep.charAt(i); // use `codePointAt` if Java 1.4.2 support is not needed.
				if (c >= min && c <= max && values[c - min] != -1) {
					throw new IllegalArgumentException(
							"The separator contains characters from the encoding alphabet");
				}
			}
		}
	...
	public byte[] decode(String in) {
		...
		while (inCount < in.length()) {
			int next = in.charAt(inCount); // use `codePointAt` if Java 1.4.2 support is not needed.
			inCount++;
